...
namespace XYFramework.Common.Encrypt
{
    ...
    {
        private static readonly Encoding encoding = Encoding.UTF32;
        ...
        private static string DiscuzAuthcode(string source, string key, DiscuzAuthcodeMode operation, int expiry)
        {
            ...
            if (operation == DiscuzAuthcodeMode.Decode)
            {
                ...
                try
                {
                    temp = Convert.FromBase64String(CutString(source, ckey_length));
                }
                catch
                {
                    try
    ...
    public class AuthEncrypt : IEncryptBase
    {
        public string Decrypt(string pToDecrypt, string sKey)
        {
            pToDecrypt = pToDecrypt.Replace("-", "+");
            pToDecrypt = pToDecrypt.Replace("_", "/");
            int mod4 = pToDecrypt.Length % 4;
            if (mod4 > 0)
            {
                pToDecrypt += "====".Substring(mod4);
            }
            pToDecrypt = Encoding.Default.GetString(Convert.FromBase64String(pToDecrypt));
            pToDecrypt = Authcode.DiscuzAuthcodeDecode(pToDecrypt, sKey);
            return pToDecrypt;
        }

        public string Encrypt(string pToEncrypt, string sKey)
        {
            pToEncrypt = Authcode.DiscuzAuthcodeEncode(pToEncrypt, sKey);
            pToEncrypt = Convert.ToBase64String(Encoding.Default.GetBytes(pToEncrypt));
            pToEncrypt = pToEncrypt.Replace("+", "-");
            pToEncrypt = pToEncrypt.Replace("/", "_");
            pToEncrypt = pToEncrypt.Replace("=", "");
            return pToEncrypt;
        }
    }
